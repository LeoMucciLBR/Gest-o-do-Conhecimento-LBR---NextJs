...
export async function PUT(
  ...
  try {
    ...
    // Atualizar pasta
    const updated = await prisma.product_folders.update({
      where: { id: folderId },
      data: {
        ...(name && { name: name.trim() }),
    ...
    console.error('Error updating product folder:', error)
    return NextResponse.json(
...
// DELETE /api/contracts/[contractId]/products/folders/[folderId] - Delete folder
export async function DELETE(
  ...
  try {
    ...
    // Check permission
    const { canDeleteProductFolder } = await import('@/lib/auth/fileAuth')
    const hasPermission = await canDeleteProductFolder(
      session.user.id,
      session.user.role,
      folderId
    )
    ...
    await prisma.product_folders.delete({
      where: { id: folderId }
    })
