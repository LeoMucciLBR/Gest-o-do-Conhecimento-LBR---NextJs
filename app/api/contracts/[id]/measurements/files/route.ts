...
// GET /api/contracts/[id]/measurements/files - List files
export async function GET(
  ...
  try {
    ...
    // Buscar arquivos
    const files = await prisma.measurement_files.findMany({
      where: {
...
// POST /api/contracts/[id]/measurements/files - Upload file
export async function POST(
  ...
  try {
    ...
    // Create directory structure: public/uploads/measurements/[contractId]
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'measurements', contractId)
    await mkdir(uploadDir, { recursive: true })
    
    const filePath = path.join(uploadDir, uniqueFilename)
    const publicUrl = `/uploads/measurements/${contractId}/${uniqueFilename}`
    ...
    const savedFile = await prisma.measurement_files.create({
      data: {
        contract_id: contractId,
        folder_id: folderId || null, // Ensure null if empty string
