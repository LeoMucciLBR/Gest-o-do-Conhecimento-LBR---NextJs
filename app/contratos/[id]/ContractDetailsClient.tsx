...
export default function ContractDetailsClient({ contractId }: ContractDetailsClientProps) {
  ...
  // Cargos de cliente (baseado no enum client_role)
  const CLIENT_ROLES = ['GESTOR_AREA', 'GERENTE_ENGENHARIA']

  // Clientes são apenas os cargos específicos de cliente
  const clienteParticipants = participants.filter((p) =>
    CLIENT_ROLES.includes(p.role.toUpperCase())
  )

  // Equipe são todos os outros cargos (COORDENADORA, ENGENHEIRO_RESPONSAVEL, GERENTE_PROJETO, ANALISTA, OUTRO)
                ...
                {sections.map((section, index) => (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-3 sm:gap-4 text-center">
                      ...
                      <span className={`font-bold text-xs sm:text-sm uppercase tracking-wider ${selectedSection === section.id ? 'text-white' : 'text-slate-700 dark:text-gray-300'
                        }`}>
                        {section.label}
                      </span>
                    </div>
        ...
        {(canEdit || canManageEditors) && (
          <div className="fixed bottom-6 left-6 z-50 sm:hidden flex flex-col-reverse items-start gap-4">
            ...
            <button
              ...
              className={`w-14 h-14 rounded-full shadow-2xl flex items-center justify-center text-white transition-all duration-300 ${isFabOpen ? 'bg-red-500 rotate-45' : 'bg-[#2f4982]'
                }`}
            >
